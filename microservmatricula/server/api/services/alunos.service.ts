...
class AlunosService {
    ...
    async publishEvent(aluno: Alunos): Promise<Alunos> {
        daprClient.pubsub.publish(process.env.APPCOMPONENTSERVICE as string, process.env.APPCOMPONENTTOPICMATRICULA as string, aluno);
    ...
    async saveNew(alunos: Alunos): Promise<Alunos> {
        ...
        await this.publishEvent(alunos);
